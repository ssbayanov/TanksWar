...
var new_mine = load("res://Objects/Hedhehog/Hedgehog.tscn")
...
func put_road(pos, tile = 10):
	...
	if tile_type:
		...
		if g.rand_rangei(0, 100) < 10:
			generate_hedhehog(pos)
...
func generate_hedhehog(pos):
	var nm = new_mine.instance()
	nm.global_position = Vector2((pos.x * 64) + 32, (pos.y * 64) + 32)
	add_child(nm)
